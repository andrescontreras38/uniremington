...
function renderPageContent(res, item) {
  ...
  const sedeDesc = sedeSlug && SEDES[sedeSlug]
    ...
    : null;
  // /donde-estamos/ traía solo un iframe de Google My Maps embebido (crudo, con un <h2>
  // "Noticias" duplicado por un artefacto de la migración de WordPress) — se reemplaza por
  // el mapa 3D propio y se conserva el resto del contenido original (noticias) tal cual,
  // cortando justo después del primer <hr> que ya separaba ambas secciones.
  const isDondeEstamos = curUrl === '/donde-estamos/';
  ...
  res.render('page', { ...base,
    ...
    contentOverride: isDondeEstamos
      ? mapaCoberturaHtml() + (item.content_html || '')
          .replace(/<div class="map-embed">[\s\S]*?<\/div>\s*/i, '')
          .replace(/^\s*<p>\s*/i, '')
      : (sedeSlug ? sedeContent(item.content_html, sedeSlug) : undefined),
